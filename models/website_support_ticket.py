...
class WebsiteSupportTicket(models.Model):
    ...
    parent_company_id = fields.Many2one(string="Empresa Socios", related="partner_id.company_id")
    partner_id = fields.Many2one('res.partner', string="Socios")
    user_id = fields.Many2one('res.users', string="Usuario asignado")
    person_name = fields.Char(string='Nombre Persona')
    email = fields.Char(string="Correo")
    support_email = fields.Char(string="Suporte Correo")
    ...
    portal_access_key = fields.Char(string="Clave de acceso al portal")
    ticket_number = fields.Char(string="Numero Ticket", readonly=True)
    ticket_color = fields.Char(related="priority_id.color", string="Color Ticket")
    ...
    close_comment = fields.Html(string="Cerrar comentario")
    close_time = fields.Datetime(string="Hora de cierre")
    close_date = fields.Date(string="Fecha Cierre")
    closed_by_id = fields.Many2one('res.users', string="Cerrado por")
    time_to_close = fields.Integer(string="Tiempo de cierre (segundos)")
    extra_field_ids = fields.One2many('website.support.ticket.field', 'wst_id', string="Extra Detalles")
    planned_time = fields.Datetime(string="Tiempo planificado")
    planned_time_format = fields.Char(string="Formato de tiempo planificado", compute="_compute_planned_time_format")
    approval_id = fields.Many2one('website.support.ticket.approval', default=_default_approval_id, string="Aprobado")
    approval_message = fields.Text(string="Mensaje de aprobación")
    approve_url = fields.Char(compute="_compute_approve_url", string="Aprobar URL")
    disapprove_url = fields.Char(compute="_compute_disapprove_url", string="Desaprobar URL")
    ...
    def update_sla_timer(self):
        ...
        # Reste 1 minuto del temporizador de todos los tickets de SLA activos, esto incluye entrar en negativo
        for active_sla_ticket in self.env['website.support.ticket'].search([('sla_active','=',True),('sla_id','!=',False),('sla_rule_id','!=',False)]):

            # Si solo hacemos la cuenta regresiva durante las horas hábiles
            if active_sla_ticket.sla_rule_id.countdown_condition == 'business_only':
                # Compruebe si la hora actual se alinea con un intervalo de tiempo en la configuración,
                # la configuración debe establecerse para business_only o se produce UserError
                ...
                # Si el módulo de vacaciones está instalado, tenga en cuenta
                ...
                #Cuenta regresiva incluso si la configuración del horario comercial no está establecida
                active_sla_ticket.sla_timer -= 1/60

            #Envíe un correo electrónico a todos en la categoría sobre la alerta de SLA
