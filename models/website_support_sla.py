...
class WebsiteSupportSLA(models.Model):
    ...
    rule_ids = fields.One2many('website.support.sla.rule', 'vsa_id', string="Reglas", help="If a ticket matches mutiple rules then the one with the lowest response time is used")
    alert_ids = fields.One2many('website.support.sla.alert', 'vsa_id', string="Alertas de correo")
